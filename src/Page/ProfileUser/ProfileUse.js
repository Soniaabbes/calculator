...
const gcPercentage = (gcCount / allBases) * 100;
    ...
    setData(prevData => ({ ...prevData, pourcentageGc: gcPercentage.toFixed(2) }));
  };
  const clearInput = (e) => {
    e.preventDefault();
...
<Form className='form' onSubmit={calculateGc} >
        <Form.Group className="mb-3" controlId="formBasicEmail">
          ...
          <Form.Control as="textarea" className='form' rows={3} name="baseNucleo" value= {data.baseNucleo} onChange={handleChange} />
          <br/>
          <br/>
          <div style={{"display":"flex", "justifyContent":"space-between", "marginLeft":"500px", "marginRight":"500px"}}>
            <Button variant= "outline-primary" type="submit">Submit</Button>
            <Button variant= "outline-primary" type="submit"  onClick={handleSubmit}>Save</Button>

            <Button variant= "outline-primary" onClick={e => clearInput(e)}>Clear</Button>
          </div>
         
      <p>GC Content: <span  className='gc' name='pourcentageGc' value= {gcResult} onChange={handleChange}>{gcResult}%  </span></p>
          <br/>
